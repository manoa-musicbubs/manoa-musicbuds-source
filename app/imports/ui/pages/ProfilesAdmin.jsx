...
import { ProfileCardAdmin } from '../components/ProfileCard';

/** Returns the Profile and associated Projects and Interests associated with the passed user email. */
function getProfileData(email) {
  const data = Profiles.findOne({ email });
  const projectPictures = data.projects.map(project => Projects.findOne({ name: project }).picture);
  // console.log(_.extend({ }, data, { interests, projects: projectPictures }));
  return _.extend({ }, data, { projects: projectPictures });
}
...
class ProfilesAdminPage extends React.Component {
  ...
  renderPage() {
    ...
    return (
        <Container>
          <Card.Group>
            {_.map(profileData, (profile, index) => <ProfileCardAdmin key={index} profile={profile}/>)}
          </Card.Group>
        </Container>
    );
  }
